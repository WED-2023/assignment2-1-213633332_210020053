// src/services/auth.js
  ...
  export async function login(credentials) {
    ...
    try {
      ...
      const response = await axios.post(`${this.$root.store.server_domain}/Login`, { username, password });
  ...
  export async function logout() {
    try {
      const response = await axios.post(`${this.$root.store.server_domain}/Logout`);
      return { status: response.status, response: { data: response.data } };
    } catch (error) {
      if (error.response && error.response.data) {
        throw { status: error.response.status, response: { data: error.response.data } };
